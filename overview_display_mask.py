...
class OverviewDisplayMask(Mask):
    ...
    def __init__(self, com_interface: SerialCommunication) -> None:
        ...
        self.temp_extruder = DataVariable(self.com_interface, 0x1000, 2, 0x2000)
        self.temp_extruder.read_data_callback = self.static_value_value_cb
        self.controls.append(self.temp_extruder)

        self.target_temp_extruder = DataVariable(self.com_interface, 0x1020,2, 0x2020)
        self.target_temp_extruder.read_data_callback = self.static_value_value_cb
        self.controls.append(self.target_temp_extruder)

        self.temp_bed = DataVariable(self.com_interface, 0x1010, 2, 0x2010)
        self.temp_bed.read_data_callback = self.static_value_value_cb
        self.controls.append(self.temp_bed)

        self.target_temp_bed = DataVariable(self.com_interface, 0x1030,2, 0x2030)
        self.target_temp_bed.read_data_callback = self.static_value_value_cb
